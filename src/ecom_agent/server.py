...
async def serve(settings: Settings, llm_client: LLMClient) -> None:
    server = grpc.aio.server()

    agent_pb2_grpc.add_AgentServiceServicer_to_server(  # type: ignore[no-untyped-call]
        AgentServicer(llm_client), server
    )
...
def main() -> None:
    ...
    settings = Settings()
    llm_client = OpenAILLMClient(AsyncOpenAI(), settings.default_model)
    asyncio.run(serve(settings, llm_client))
